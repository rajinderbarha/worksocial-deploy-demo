...
// function LocationCard({office}:any) {
//     console.log(office);
    

//     const{image:{data}} = office
//   return (
//    <>
...
//             <Image src={getStrapiMedia(data[0].url)} height={378} width={488} alt="" className='w-100 h-100'/>
//         </div>
//     </div>
...
function LocationCard({ office }: any) {
    const { image: { data }, city: { CityName }, spaceName, price, duration, spaceCategory } = office;
    // console.log(office);
            ...
            <div className="row gx-0">
                ...
                <div className="col-md-7">
                    <div className={`${classes.LocationCard_content} d-flex flex-column justify-content-between h-100`}>
                        <div className={`d-flex justify-content-between`}>
                            <div className={`${classes.LocationCard_text} font-inter`}>
                                <p className={`${classes.LocationCard_text_city} subtitle font-normal`}><span>{spaceCategory} </span><span>{CityName}</span></p>
                                <p className='text_body dark font-semibold '> {spaceName}</p>
                            </div>
                            <div className={`${classes.LocationCard_bookmark_icon}`}>
                                <IoBookmarkOutline color='grey_mid' />
                            </div>
                        ...
                        <div className={`font-inter text-end`}>
                            <p className={`${classes.pricetext} font-normal`}>Starting at</p>
                            <h6 className={`${classes.price_number} price dark font-semibold`}><span className={`${classes.pricetext_dollor}`}>$</span>{price}<span className={`${classes.price_time} font-semibold `}>{duration}</span></h6>
                        </div>
                    </div>
                </div>
            </div>
